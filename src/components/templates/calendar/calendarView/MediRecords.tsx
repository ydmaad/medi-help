...
import {MOCK_DATA} from "@/components/templates/calendar/calendarView/CalendarView";
        ...
        <div className="border border-[#F5F6F7] bg-[#FBFBFB] p-5">
          <div className="flex justify-between items-center mb-4">
            <div className="flex justify-between gap-6 text-[16px] font-normal">
              <div className={`cursor-pointer ${tabNumber === 0 ? "font-bold" : "text-[#7C7F86]"}`}
                   onClick={() => setTabNumber(0)}>복약 리스트
              </div>
              <div className={`cursor-pointer ${tabNumber === 1 ? "font-bold" : "text-[#7C7F86]"}`}
                   onClick={() => setTabNumber(1)}>노트
              </div>
            </div>
          ...
          {tabNumber === 0 ?
              <>
                ...
                <div className="flex flex-col w-full gap-2">
                  {MOCK_DATA.filter(e => e.timeOfDay === timeOfDay).map(ele => <PillComponent key={ele.pillName}
                                                                                              pill={ele}/>)}
                </div>
              </> : null
          }
          {tabNumber === 1 ?
              <textarea className="min-h-[125px] p-4 w-full text-[16px] font-normal resize-none"
                        value="신경불안약 먹으니까 너무 졸림. 약 복용 주기 4시간 지키면서 복용하기"></textarea> : null
          }
        </div>
